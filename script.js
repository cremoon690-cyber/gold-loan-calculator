...
(() => {
  ...
  function setResultRingPercent(pct) {
  ...
  const val = Math.max(parseFloat(sliderEl.value) || min, min);
  ...
  //   setResultRingPercent(sliderPercent(elements.loanAmountSlider));
  // }
  ...
  const loanAmountPercent =
    ((state.loanAmount - CONFIG.limits.loanAmount.min) /
      (CONFIG.limits.loanAmount.max - CONFIG.limits.loanAmount.min)) * 100;

  setResultRingPercent(loanAmountPercent);
}
  ...
  // FIXED: typing no longer force-clamps to min/max on every keystroke.
  // Clamping now happens only on blur (when the user leaves the field),
  // so the input can be freely typed into, cleared, or edited.
  function bindWeightInputs() {
    ...
    // typing: live-update only if the typed value is already valid,
    // otherwise leave the field alone so the user can keep typing
    ...
    // finalize / clamp only once the user leaves the field
    elements.weightInput.addEventListener("blur", (event) => {
      commitWeightValue(event.target.value);
    });

    // slider drag is always within range, so it can commit instantly
    elements.weightSlider.addEventListener("input", (event) => {
      commitWeightValue(event.target.value);
    });
  }

  // FIXED: same typing-clamp fix applied here for loan amount,
  // interest weight, interest rate, and tenure fields.
  function bindSyncedInputs(input, slider, stateKey, min, max, callback) {
    ...
    // typing: live-update only when valid, never force min/max mid-type
    ...
    // clamp only once the user leaves the field
    input.addEventListener("blur", (event) => {
      commitValue(event.target.value);
    });

    // slider drag commits instantly (always in range)
